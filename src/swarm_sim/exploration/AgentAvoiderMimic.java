...
public class AgentAvoiderMimic extends DefaultExplorationAgent implements
    ...
    ScanCircle agentRepellingScan = new ScanCircle(8, 1, 1,
	    ...
	    GrowingDirection.Inner, 0, 0.8 * scenario.commScope, 0.5, 2);
    ScanCircle agentAppealingScan = new ScanCircle(8, 1, 1.5,
	    AttractionType.Appealing, DistributionType.Linear,
	    GrowingDirection.Outer, 0.9 * scenario.commScope,
	    scenario.commScope, 0.5, 2);
    ScanCircle agentMimicMoveScan = new ScanCircle(8, 1, 1,
	    AttractionType.Appealing, DistributionType.Linear,
	    GrowingDirection.Outer, 0, scenario.commScope, 0.5, 0.5);
	...
	if (state == agentState.exploring) {
	    ...
	    ScanCircle resulting = ScanCircle.merge(8, 0.12,
		    agentAppealingScan, agentRepellingScan, agentMimicMoveScan,
		    obstacles);
	...
	for (IAgent agent : commNet.getAdjacent(this)) {
		...
		agentRepellingScan.add(agentRepellingScan.new InputPair(angle,
			distance));
		agentAppealingScan.add(agentAppealingScan.new InputPair(angle,
			distance));
		break;
	    default:
		break;
