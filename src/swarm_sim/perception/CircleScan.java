...
public class CircleScan {
	...
	public double getRandomAngle() {
		...
		if (rndSum >= rndS) {
		    return RandomHelper.nextDoubleFromTo(s.start, s.end);
		}
	    }
     ...
     * @return
     */
    public static CircleScan merge(int segmentCount, double surpressRandom,
	    List<AngleSegment> freeToGoSegments, CircleScan... scans) {
	...
	for (int i = 0; i < segmentCount; i++) {
		...
		weightedDataSum += cs.mergeWeight
			* cs.segments.get(i).segmentWidthSum
			* cs.segments.get(i).value;
	    }

	    if (mergeFactorSum > 0) {
		if (merged.segments.get(i).isAllowed)
		    merged.segments.get(i).value = weightedDataSum
			    / (mergeFactorSum * segmentWidthSum);
		else
