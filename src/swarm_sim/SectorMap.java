...
public class SectorMap {
	...
	public SectorMap(Dimensions dims, int sectorsX,
			int sectorsY, int degree) {
		init(dims, sectorsX, sectorsY, degree);
	}

	public SectorMap(SectorMap parent, int x, int y, Dimensions dims, int sectorsX,
			int sectorsY, int degree) {
		this.parent = parent;
		this.x = x;
		this.y = y;
	...
	private void init(Dimensions dims, int sectorsX,
			int sectorsY, int degree) {
		this.dims = dims;
		...
		if((sectorsX == 1 && sectorsY == 1) || degree == 0)
			...
			for (int x = 0; x < sectorsX; x++) {
				for (int y = 0; y < sectorsY; y++) {
					map[x][y] = new SectorMap(this, x,y,new Dimensions(dims.getWidth()/sectorsX, dims.getHeight()/sectorsY), sectorsX, sectorsY, degree-1);
				}
			}
		}
	}
	...
	public double getNewMoveAngle() {
		if(!map[targetX][targetY].isFilled()) {
			if(posX != targetX || posY != targetY)
				return SpatialMath.angleFromDisplacement(targetX - posX, targetY - posY);
	...
	public void chooseNewTargetSector() {
		...
		int depth = Math.max(Math.abs(targetX - newTarget.x), Math.abs(targetY - newTarget.y));
		...
		newTarget = neighbors.get(RandomHelper.nextIntFromTo(0, neighbors.size()-1));
		targetX = newTarget.x;
		targetY = newTarget.y;
	}
